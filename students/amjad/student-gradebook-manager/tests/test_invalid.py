...
def test_add_student_short_name(students):
    with pytest.raises(ValidationError, match="Name must be at least 2 characters long"):
        add_student(students, "b", "bakro@example.com", 25)
...
def test_add_student_invalid_age(students):
    with pytest.raises(ValidationError, match="Age must be between 16 and 60"):
...
def test_add_grade_invalid_score(students):
    with pytest.raises(InvalidGradeError, match="Score must be between 0 and 100"):
